...
struct nsenum_entry
{
  ...
  nsenum_entry(std::string iname, std::string ivalue, bool iflag_is_0 = false,
               bool write_enum_val = false)
  {
    name                  = std::move(iname);
    string_value          = name;
  ...
  std::string get_value(std::vector<nsenum_entry> const& entries,
...
struct nstuple_type
{
  ...
  {
    if (type == "String" || type == "StringView" || type == "std::string" ||
...
struct nsenum_context
{
  ...
  static void begin_header(std::ostream& ofs, std::vector<std::string> const&);
  static void start_namespace(std::ostream& ofs, std::string);
  static void end_namespace(std::ostream& ofs, std::string const&);
  static void add_includes(std::ostream& ofs, std::vector<std::string> const&);
  static std::string prefix_words(std::string const& pref,
                                  std::string const& sentence,
                                  bool               is_single_word);
  static std::string modify_search(std::string const& on,
                                   nsenum_modifier    modifier);

  static void parse(std::string const& mod, std::string const&   header,
...
struct nsenum
{
  ...
  std::string              namespace_name = "lumiere";
  std::string              comment;
  ...
  nsenum(nsenum_context const& ctx, nlohmann::json const&);

  inline std::string enum_name() const { return name; }

  static auto sorted_tuple(std::string const& s) 
  {
    return std::make_pair<int, int>(-(int)s.length(), (int)s.length());
  }
