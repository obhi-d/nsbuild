...
void foreach_variable(std::ostream& ostr, std::string_view content, Lambda&& l)
{
  ...
  while (r < content.length())
  {
    ...
    if (pos != content.npos)
    {
      if (pos == content.length() - 1 || content[pos + 1] == '$' || content[pos + 1] == '{')
      {
        off = pos + 1;
        continue;
      }
...
inline std::string target_name(std::string_view fw, std::string_view mod) { return fmt::format("{}.{}", fw, mod); }
